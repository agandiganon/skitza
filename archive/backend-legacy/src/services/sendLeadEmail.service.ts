...
import { getResendClient } from "./resend.service.js";

const FALLBACK_LEAD_RECIPIENT = "adi181800030@gmail.com";
...
export async function sendLeadEmail(payload: ContactPayload): Promise<void> {
  ...
  const to = env.RESEND_TO_EMAIL || FALLBACK_LEAD_RECIPIENT;
  ...
  const text = [
    "פנייה חדשה התקבלה מאתר סקיצה אריזות",
    "",
    `שם: ${safePayload.name}`,
