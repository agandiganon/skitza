...
const items = [
  {
    ...
    description:
      'שירותי ייעוץ ותכנון אריזות/קרטוניות ברמה גבוהה, בהתאמה למוצר, למותג ולתקציב.',
  },
  {
    icon: Palette,
    title: 'ביצוע פרויקטים מורכבים',
    description:
      'ניסיון מעשי בפרויקטים מורכבים כולל אריזה, עם תהליך עבודה מסודר וליווי רציף.',
  },
  {
    icon: Headphones,
    title: 'רמת גימור מושלמת',
    description:
      'בקרת איכות קפדנית לאורך כל הדרך כדי להבטיח תוצאה סופית מדויקת ומרשימה.',
  },
];
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        <div className="absolute top-8 -right-28 h-64 w-64 rounded-full bg-blue-300/25 blur-3xl" />
        <div className="absolute bottom-10 -left-24 h-72 w-72 rounded-full bg-cyan-300/20 blur-3xl" />
      </div>
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mb-12 text-center sm:mb-14">
          <h2
            ...
            className="text-primary text-4xl font-black tracking-tight sm:text-5xl lg:text-[3.35rem]"
          >
            היתרונות שלנו
          </h2>
          <p className="text-foreground/75 mx-auto mt-4 max-w-3xl text-base leading-relaxed sm:text-lg">
            שילוב בין קריאייטיב, דיוק הנדסי וחומרי גלם איכותיים כדי שהאריזה שלכם
            תבלוט, תישאר עמידה ותשדר מותג חזק בכל מפגש עם הלקוח.
          </p>
        </div>
        ...
        <div className="mb-12 grid gap-7 sm:grid-cols-3 lg:mb-14">
          {items.map((item, i) => (
            <article
              key={item.title}
              className="group relative flex h-full flex-col items-center overflow-hidden rounded-[28px] border border-blue-200/80 bg-white/85 p-8 text-center shadow-[0_18px_45px_-26px_rgba(30,58,95,0.7)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-1.5 hover:border-blue-300 hover:shadow-[0_26px_60px_-30px_rgba(30,58,95,0.75)]"
            >
              ...
              <span className="absolute top-4 left-4 inline-flex h-8 min-w-8 items-center justify-center rounded-full bg-blue-100 px-2 text-xs font-bold text-blue-700">
                0{i + 1}
              </span>
              <div className="from-accent-cyan to-primary relative mb-5 inline-flex rounded-2xl bg-gradient-to-l via-blue-500 p-4 text-white shadow-lg ring-4 ring-blue-100/70 transition duration-300 group-hover:scale-[1.06] group-hover:shadow-xl">
                <item.icon className="h-8 w-8" aria-hidden />
              </div>
              <h3 className="text-primary mb-2.5 text-xl leading-tight font-extrabold">
                {item.title}
              </h3>
              <p className="text-foreground/78 leading-relaxed">
                {item.description}
              </p>
            </article>
          ))}
        ...
        <div className="relative mx-auto max-w-6xl overflow-hidden rounded-[30px] border border-blue-200/80 bg-white/90 p-5 shadow-[0_24px_70px_-40px_rgba(29,78,216,0.6)] sm:p-7 lg:p-8">
          ...
          <div className="relative grid gap-4 sm:grid-cols-3">
            {PROCESS_STEPS.map((step) => (
              ...
              >
                <div
                  className="from-accent-cyan to-primary mb-3 h-1.5 w-14 rounded-full bg-gradient-to-l via-blue-500"
                  aria-hidden
                />
        ...
        <p className="mt-9 text-center">
          <Link
            ...
            className="from-accent-cyan to-primary focus:ring-primary inline-flex min-h-[50px] items-center justify-center gap-2 rounded-2xl bg-gradient-to-l via-blue-500 px-8 py-4 text-lg font-semibold text-white shadow-lg transition duration-300 hover:-translate-y-0.5 hover:shadow-xl focus:ring-2 focus:ring-offset-2 focus:outline-none"
          >
            קבלת הצעת מחיר
