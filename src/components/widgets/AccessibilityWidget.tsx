...
function isTextScaleLevel(value: unknown): value is TextScaleLevel {
  return (
    typeof value === 'number' &&
    TEXT_SCALE_LEVELS.includes(value as TextScaleLevel)
  );
}
...
export function AccessibilityWidget() {
  ...
  const adjustTextScale = (direction: 'up' | 'down') => {
    const currentIndex = TEXT_SCALE_LEVELS.findIndex(
      (level) => level === settings.textScale,
    );
    const delta = direction === 'up' ? 1 : -1;
    const nextIndex = Math.min(
      TEXT_SCALE_LEVELS.length - 1,
      Math.max(0, currentIndex + delta),
    );
    setSettings((prev) => ({
      ...prev,
      textScale: TEXT_SCALE_LEVELS[nextIndex],
    }));
  };
  ...
  return (
    <div
      ...
      className="fixed bottom-[calc(var(--floating-stack-offset)+0.45rem)] left-3 z-[64] sm:bottom-6 sm:left-6"
    >
      {settings.largeCursor ? (
      ...
      <button
        ...
        className="bg-primary/94 flex min-h-[48px] min-w-[48px] items-center justify-center rounded-full border border-blue-100/80 text-white shadow-[0_12px_28px_-16px_rgba(15,37,64,0.42)] backdrop-blur-sm transition hover:scale-105 hover:shadow-[0_18px_38px_-16px_rgba(15,37,64,0.52)] focus:ring-2 focus:ring-blue-300 focus:ring-offset-2 focus:outline-none"
      >
      ...
      {open ? (
        <div
          ...
          className="mt-3 max-h-[min(72vh,560px)] w-[min(92vw,360px)] overflow-y-auto rounded-2xl border border-blue-100 bg-white/95 p-4 shadow-[0_20px_50px_rgba(15,37,64,0.26)] backdrop-blur-md"
        >
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-foreground/70 mt-1 text-xs">
                התאמות נגישות לצפייה נוחה יותר
              </p>
            </div>
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="text-foreground/70 hover:text-primary rounded-lg p-2 transition hover:bg-blue-50 focus:ring-2 focus:ring-blue-300 focus:outline-none"
          ...
          <section className="mb-4 rounded-xl border border-blue-100 bg-blue-50/55 p-3">
            <div className="mb-2 flex items-center justify-between">
              <p className="text-primary text-sm font-bold">גודל טקסט</p>
              <p className="text-primary rounded-md bg-white px-2 py-1 text-xs font-semibold">
                {settings.textScale}%
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-primary inline-flex min-h-[44px] flex-1 items-center justify-center rounded-lg border border-blue-200 bg-white transition hover:bg-blue-100 focus:ring-2 focus:ring-blue-300 focus:outline-none disabled:cursor-not-allowed disabled:opacity-45"
              ...
              return (
                <li key={item.key}>
                  <button
                    ...
                    className={`flex min-h-[44px] w-full items-center gap-3 rounded-xl border px-3 py-2 text-right transition focus:ring-2 focus:ring-blue-300 focus:outline-none ${
                      isActive
                        ? 'text-primary border-blue-500 bg-blue-50'
                        : 'text-foreground border-blue-100 bg-white hover:border-blue-200 hover:bg-blue-50/50'
                    }`}
                  >
                    <span
          ...
          <button
            ...
            className="mt-4 inline-flex min-h-[46px] w-full items-center justify-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-sm font-bold text-rose-700 transition hover:bg-rose-100 focus:ring-2 focus:ring-rose-300 focus:outline-none disabled:cursor-not-allowed disabled:opacity-45"
          >
